 var DEFAULT_QUIZ_ITEM_TEXT = 'Double-click here to start writing quiz item text...'; // also kept in quiz_editor js
...
var input=element.apply(form,[settings,self]);var input_content; if (self.revert == DEFAULT_QUIZ_ITEM_TEXT) self.revert = ''; self.revert = self.revert.split('<br>').join('\n').replace(/<span id="blank">[^<]+<\/span>/g, '??');  if(settings.loadurl){var t=setTimeout(function(){input.disabled=true;content.apply(form,[settings.loadtext,settings,self]);},100);var loaddata={};loaddata[settings.id]=self.id;if($.isFunction(settings.loaddata)){$.extend(loaddata,settings.loaddata.apply(self,[self.revert,settings]));}else{$.extend(loaddata,settings.loaddata);}
