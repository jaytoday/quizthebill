...
var jscolor = {
	...
	bind : function() {
		...
		for(var i=0; i<e.length; i++) {
			var m
			if(e[i].className && (m = e[i].className.match(matchClass))) {
				var prop = {}
				if(m[3]) {
					try {
						eval('prop='+m[3])
	...
	fireEvent : function(el, evnt) {
		...
		} else { // alternatively support the traditional event model (IE5)
			if(el['on'+evnt]) {
				return el['on'+evnt]()
			} else {
				return false
	...
	color : function(target, prop) {
		...
		this.toString = function() {
			return (
				(0x100 | Math.round(this.rgb[0]*255)).toString(16).substr(1) +
		...
		function showPicker() {
			if(!isPickerOwner()) {
				...
				var vp = jscolor.getViewPos(target) // view pos
				var vs = jscolor.getViewSize(target) // view size
				var ps = [ // picker size
				...
				var pp = [ // picker pos
					-vp[0]+tp[0]+ps[0] > vs[0] ? (-vp[0]+tp[0]+ts[0]/2 > vs[0]/2 && tp[0]+ts[0]-ps[0] > 0 ? tp[0]+ts[0]-ps[0] : tp[0]) : tp[0],
					-vp[1]+tp[1]+ts[1]+ps[1] > vs[1] ? (-vp[1]+tp[1]+ts[1]/2 > vs[1]/2 && tp[1]-ps[1] > 0 ? tp[1]-ps[1] : tp[1]+ts[1]) : tp[1]+ts[1]
				]
